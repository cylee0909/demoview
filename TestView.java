...
public class TestView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        amplitude -= (ratio * (width >> 3) * Math.random());
        if (amplitude <= 0 || amplitude >= (width >> 3)) {
            ratio *= -1;
        }
        //绘制背景
        ...
        for (int i = 0; i <= SAMPLING_SIZE; i++) {
            ...
            curV = nextV;
            //提前算出下一采样点的值
            nextV = i < SAMPLING_SIZE ? (float) (amplitude * calcValue(mapX[i + 1], 0.5f)) : 0;
